...
- **Absolute mode tracks position in-process.** For cameras without `pan_relative`, we send `pan_absolute=<current ± step>` and clamp to detected min/max. The tracked position lives in an `AtomicI32` per axis. If another tool also drives the camera concurrently, our tracking diverges from reality — `home()` writes 0 directly to recover.
